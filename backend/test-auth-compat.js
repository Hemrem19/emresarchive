/**
 * Authentication API Test Script (Compatible with Node.js < 18)
 * Run with: node test-auth-compat.js
 * 
 * This version uses node-fetch instead of native fetch
 */
...
// HTTP request helper using Node.js built-in http/https
  ...
  return new Promise((resolve, reject) => {
    ...
    req.on('error', (error) => {
      if (error.code === 'ECONNREFUSED') {
        resolve({ 
          status: 0, 
          error: `Connection refused. Is the server running at ${API_URL}?` 
        });
      } else {
...
async function runAllTests() {
  ...
  const healthCheck = await makeRequest('GET', '/health'.replace('/api/auth', ''));
