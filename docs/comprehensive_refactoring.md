...
Document Type: Competitive Intelligence & Product Architecture Analysis
Objective: Deconstruct the collaborative capabilities of Zotero Groups and alternative market solutions to inform the development of our own native collaborative reference management infrastructure.
...
The most dangerous moment in the Citavers user journey is the "First-Time Login".
A user might have utilized the platform unauthenticated for weeks, accumulating 50 papers in their local IndexedDB. When they finally click "Login" and authenticate, the system must merge their local state with the Cloud state.
...
The Engineering Challenge: As identified earlier, using a Deterministic ID solves the duplication problem for a single user merging their own states. However, what happens in a Shared Collaborative Workspace?
If User A and User B both independently add the exact same paper (same DOI hash) to a shared group, the CRDT will flawlessly merge them because the IDs match.
...
14. Deep Dive: Paper Membership & State Propagation Matrix

The architectural shift to deterministic hash(DOI) identifiers (Section 13-C) creates a profound new capability: papers are no longer strictly "trapped" in isolated silos like they are in Zotero. They exist as universally recognized entities.

However, this raises complex questions regarding Membership (Where does the paper live?) and State Propagation (If it updates in one place, does it update everywhere?).

To resolve this, Citavers must implement a strict Tripartite Membership Architecture.

A. The Tripartite Membership Architecture

We must mathematically define the relationship between a paper and the user interface. We divide this into three layers: Platform, Workspace, and Collection.

The Platform Level (The Core Entity)

Status: A paper (hash(DOI)) exists at the Platform level the moment any user in the entire Citavers ecosystem adds it.

Ownership: It belongs to the database, not to any specific user. It holds only immutable universal metadata (Title, Year, Journal, Authors).

The Workspace Level (The "Claim" / Contextual Node)

Status: A Workspace (a User's Private Library or a Shared Lab Group) issues a "Claim" on a Core Entity. It states: "This workspace is actively referencing this paper."

Ownership: This layer holds the Contextual Node (meta:doi_hash:workspace_id). This is where the Workspace-specific data lives: Status (To Read, Finished), custom Tags, Ratings, and the Discussion Drawer content.

PDF Attachment Rule (DMCA Compliance): Crucially, PDF file references belong to the Workspace Claim, NOT the Core Entity. If User A uploads a PDF to their private workspace, User B (who claimed the same paper in their own workspace) cannot see the PDF. This isolates copyright liability and storage quotas to the specific Workspace owner.

The Collection Level (The "View" / Pointer)

Status: Within a Workspace, a user creates Collections (Folders) like Thesis 2026 or Machine Learning. A Collection does not own a paper. It is merely a relational list of pointers (tags) that say "Include paper X in this view."

Ownership: Collections own nothing. They are purely visual filters applied over the Workspace's claimed papers.

B. State Propagation Rules (What Updates Where?)

When a paper exists in multiple Workspaces (e.g., User A's private library and User A's shared lab group), changes must propagate according to strict boundaries.

Core Metadata Updates (Global Propagation):

Scenario: User A realizes the abstract for a paper is missing and clicks "Fetch updated metadata from CrossRef."

Propagation: Because this edits the Core Entity, the updated abstract instantly propagates to every Workspace in Citavers that has claimed this hash(DOI).

Architectural Optimization: Because Core Metadata updates globally and is rarely edited collaboratively in real-time, it does not need to be stored inside the heavy Yjs CRDT payload. It can be stored as standard relational data in Cloudflare D1. The client simply fetches the latest Core Metadata via a standard query, keeping the CRDT payload lean and focused strictly on collaborative notes/tags.

Contextual Updates (Isolated Propagation):

Scenario: User A changes the paper status to Finished and adds the tag #Boring in their Private Workspace.

Propagation: These edits occur on the meta:doi_hash:workspace_id node. They do not propagate to the Shared Lab Group, even if the lab group contains the exact same paper. The contexts remain mathematically isolated.

C. The Deletion Cascade (Orphan Prevention)

How deletions are handled determines the integrity of the user's library. Citavers must implement a specific cascading logic based on the Tripartite model.

Deleting from a Collection (Safest Action):

Action: User removes a paper from the Machine Learning folder.

Result: Only the pointer in the Collection object is removed. The paper remains fully intact within the Workspace's "All Papers" view.

Deleting from a Workspace (The Context Destroyer):

Action: User deletes the paper entirely from their Private Library.

Result: The system destroys the Workspace's "Claim" and permanently deletes the Contextual Node (tags, notes, status, and PDF reference) associated with that workspace. Warning: If this was the only copy of the PDF, the PDF is deleted from Cloudflare R2 to reclaim quota.

The Core Entity Survival: The Core Entity (hash(DOI)) is not deleted from the database as long as at least one other Workspace in the Citavers ecosystem still claims it.

Platform Level Deletion (Garbage Collection):

Action: The backend Cron Job scans the database.

Result: If a Core Entity (hash(DOI)) has zero associated Workspace Claims (i.e., every user who ever had this paper deleted it), the system permanently deletes the Core Entity from Cloudflare D1 to prevent long-term database bloat.

D. Implementation in the Cloudflare D1 Architecture

To implement this membership matrix efficiently using our proposed Cloudflare infrastructure:

The Shared Workspace Durable Object: The Yjs document living in the Durable Object will maintain a Y.Map or Y.Array representing its "Claims" (the list of hash(DOI) strings it currently tracks).

The Client-Side Join: When the Vanilla JS frontend connects to a Workspace, it performs a local join. It downloads the CRDT state from the Durable Object (which contains the tags and notes) and simultaneously queries the D1 database for the Core Metadata of the claimed hash(DOI) list. The UI then stitches them together into a single "Paper Card" for the user.

15. Deep Dive: CRDT Garbage Collection & State Snapshotting
...
16. Tech Stack Mapping & Cost Infrastructure (Citavers Context)
...
Backend Cron Jobs (The Garbage Collector): You need an Express.js cron job (e.g., using node-cron) running nightly at 03:00 AM to perform the Snapshotting and 30-Day Tombstone Pruning mentioned in Section 15.
...
17. The Edge Computing Pivot (Cloudflare Workers + D1 Migration)

The limitations outlined in Section 16—specifically the Railway WebSocket RAM costs and the Neon.db 100-hour compute limit—present significant scaling risks. To completely eliminate these "Always-On" costs and optimize for a global collaborative architecture, migrating the entire backend to Cloudflare Workers and Cloudflare D1 (Serverless SQLite) is a strategically superior move.
...
The Express cron job (Section 16-C) must be refactored for the Cloudflare ecosystem.
...
18. Database Migration Plan (Prisma to CRDT/D1)
...
19. Security Vulnerabilities & Threat Mitigation (The Zero-Trust Model)
...
20. Final Assessment & Next Steps for Engineering
...
Master the Tripartite Membership & Merge Logic: * Implement deterministic, DOI-based hashing for Core Paper IDs to ensure universal metadata propagation across workspaces.

Implement Composite IDs (doi_hash:workspace_id) for Contextual Nodes (tags/notes) to isolate personal/group context.

Bind PDF attachment storage strict to the Workspace Claim, not the Core Entity, to satisfy DMCA rules.
